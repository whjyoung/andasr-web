	...
	export default {
		...
		data() {
			return {
				...
				workstation: false, //确定是 产品 
				total: 0, //默认数据总数
				pagesize: 10, //默认每页的数据条数
				currentPage: 1, //默认开始页面
				wordType:'0',//给Schart一个标识，0为产品页，1为工位页
		...
		methods: {
			...
			/*给TableList查询资源*/
			getTableList(_arry) {
				api.getProductSource(_arry).then(res => {
					if(res.code == '0') {
						//table列表
			...
			/*本页面所有资源*/
			getAllSource(_arry) {
				api.getProductSource(_arry)
					.then(res => {
						if(res.code == '0') {
							this.stationId = res.data.word;
							//table列表
							...
							this.label1 = [];
							this.data1[0].data = [];
							for(let i = 0; i < res.data.graphSList.length; i++) {
								this.data1[0].data.push(res.data.graphSList[i].num);
								this.label1.push(res.data.graphSList[i].stationId);
								this.schart2 = true;
							}
						} else {
							$ui.warnMsg(res.message);
